---
name: graphify
description: "Queryable dependency graph of this repository (graphify-out/graph.json). Use SELECTIVELY for impact analysis before changing a shared symbol (what depends on X, which tests cover it), finding the callers or wiring site of a function, and orienting in an unfamiliar subsystem, via `graphify affected`, `graphify explain`, `graphify path`. Skip it for scoped edits where the file and symbol are already known. A discovery aid only: the source code is authoritative. Also handles `/graphify` (build or refresh the graph). Narrowed for this repo by Phase 17 tooling; a re-run of `graphify install --project` overwrites this line."
---
