...
def test_estimator_metadata_qualifies_nonconvergence_alias_and_selector(
    ...
    assert len(rows) == 14
    # --- Owner Addendum 07C: frozen golden value re-pinned -------------------
    #
    # This literal is a REGRESSION PIN captured from a campaign run, not a
    # physical invariant. It was generated by the pre-repair measurement model,
    # so the Addendum-05 time-conditioning repair necessarily moved it:
    #
    #     pre-repair  : 0.009216843912656895
    #     post-repair : 0.018747738770222504
    #
    # Evidence gathered before changing it (Addendum-07 section 8 conditions):
    #   * deterministic -- two independent runs reproduce it bit-identically;
    #   * the quantity is a normalized ESTIMATE SHIFT, and both the old and the
    #     new value sit far below the accepted 0.1 sigma NEGLIGIBLE materiality
    #     band, so no materiality classification changes;
    #   * the shift follows from the repaired observable being closer to
    #     independent truth, evidenced in r3_model_s_independent_oracle.csv.
    #
    # The pin is kept exact so a genuine future regression still trips it, and
    # is backed by an explicit materiality bound so the pin can never drift
    # into scientifically meaningful territory unnoticed.
    R2_ESTIMATE_SHIFT_SIGMA_PIN = 0.018747738770222504
    R2_ESTIMATE_SHIFT_MATERIALITY_SIGMA = 0.1
    observed_shift_sigma = max(
        abs(float(row["estimate_shift_over_reference_posterior_sigma"]))
        for row in rows
    )
    assert observed_shift_sigma == R2_ESTIMATE_SHIFT_SIGMA_PIN
    assert observed_shift_sigma < R2_ESTIMATE_SHIFT_MATERIALITY_SIGMA, (
        observed_shift_sigma,
        R2_ESTIMATE_SHIFT_MATERIALITY_SIGMA,
    )
    for row in rows:
...
# --- Owner Addendum 07: R3 numerical-oracle authority transition -------------
#
# WHAT CHANGED AND WHY (the short version; full evidence in the Addendum-07
# artifact bundle, R3_MODEL_S_GATE_REINTERPRETATION.md):
#
# P04A/P04B/P11 above compare repaired production against the accepted model-S
# reference. Model-S builds its round-trip light time as
#
#       round_trip_light_time_s = float(t3 - t1)          (reference, line 466)
#
# and PRE-REPAIR production built it the same way
#
#       round_trip_light_time_s = t3 - t1                 (canonical ec4b6871)
#
# so both sides carried the SAME ulp(pass-relative epoch) conditioning error and
# it cancelled exactly in the difference. The agreement these gates measured was
# therefore AGREEMENT OF ARITHMETIC ROUTE, not agreement with physical truth.
#
# The Addendum-05 repair assembles production's round-trip light time from the
# well-conditioned local intervals instead:
#
#       round_trip_light_time_s = downlink_lt + delay + uplink_lt
#
# Model-S is byte-protected and keeps the old route, so the difference these
# gates now see is model-S's own conditioning error, exposed rather than
# cancelled. An independent high-precision oracle (60-digit Decimal on the local
# unknowns, cross-checked against a scipy brentq solve of the same local
# unknowns) confirms repaired production is closer to truth than model-S on
# every sampled row, by factors of 1.2x to 1.8e3x.
#
# Consequently model-S is NO LONGER the primary NUMERICAL-ACCURACY oracle for
# these quantities. It remains in the suite as a HISTORICAL CHARACTERIZATION
# reference, byte-immutable, and its divergence is recorded rather than hidden.
#
# NOTHING PHYSICAL MOVED. The 1e-11 s physical equation residual criterion, the
# event-update criterion, the count-interval convention, the delay convention
# and the speed of light are all unchanged. This is an oracle-authority
# transition, not a tolerance loosening.

# Derived from the repaired arithmetic, NOT chosen to make a test pass:
# tau_down and tau_up each carry one norm+divide rounding, the round-trip
# assembly adds two more, and the endpoint difference one -> 5 ulp(rho).
R3_A07_ORACLE_K_ULP_RHO = 5.0

# Historical-characterization headroom for the model-S comparison. This is NOT
# an accuracy criterion: it is the observed scale of model-S's own epoch
# conditioning error, kept so that a NEW production regression still trips the
# check while the known model-S drift does not. Accuracy is gated separately,
# against the independent oracle, by the A07 tests below.
R3_A07_MODEL_S_CHARACTERIZATION_K = 512.0
...
# --- Owner Addendum 07: independent numerical-truth oracle -------------------
#
# INDEPENDENCE, STATED PRECISELY (do not overstate it):
#
#   independent  : the EVENT ARITHMETIC. Neither production nor model-S is
#                  called. The local light-time unknowns tau_down and tau_up
#                  are solved directly from their defining equations with a
#                  scalar bracketing root finder.
#   NOT independent: the EPHEMERIS / STATE TRUTH. Station states come from the
#                  accepted exact-event provider and spacecraft states from the
#                  accepted cubic-Hermite interpolation -- the same sources
#                  production uses. That sharing is deliberate: the quantity
#                  under test is the conditioning of the event arithmetic, so
#                  holding the state source fixed is what makes it a controlled
#                  comparison.
#
# This binary64 brentq route was validated in the Addendum-07 bundle against a
# 40/60/80-digit Decimal fixed-point solve of the same local unknowns
# (independent_oracle_validation.csv, 10/10 checks, agreement 0.0 s).
def _r3_independent_round_trip_light_time(fixture, provider, t3_s):
    """Round-trip light time from a scalar root solve on LOCAL unknowns."""
    from scipy.optimize import brentq

    t3 = float(t3_s)
    c = campaign.C_LIGHT_MPS
    station_rx = provider.state(t3)

    def downlink_residual(tau):
        sc = radiometrics_module._interp_state(
            fixture.t_grid_s, fixture.state_history_mci, t3 - tau
        )
        return tau - float(np.linalg.norm(sc[:3] - station_rx[:3])) / c

    tau_down = brentq(downlink_residual, 0.5, 6.0, xtol=1e-15, rtol=8.9e-16, maxiter=300)
    t2 = t3 - tau_down
    sc_t2 = radiometrics_module._interp_state(
        fixture.t_grid_s, fixture.state_history_mci, t2
    )

    def uplink_residual(tau):
        g1 = provider.state(t2 - tau)
        return tau - float(np.linalg.norm(sc_t2[:3] - g1[:3])) / c

    tau_up = brentq(uplink_residual, 0.5, 6.0, xtol=1e-15, rtol=8.9e-16, maxiter=300)
    return tau_down + tau_up


def _r3_independent_observable(fixture, provider, interval_s):
    """Zero-delay counted-Doppler observable from the independent oracle.

    Convention mirrored from production: endpoints at t_mid -/+ Tc/2 on the
    ground-receive epoch, rho_rate = (rho_end - rho_start)/Tc, h = c*rho_rate/2.
    """
    mid = float(fixture.spec.receive_mid_time_s)
    half = 0.5 * float(interval_s)
    rho_start = _r3_independent_round_trip_light_time(fixture, provider, mid - half)
    rho_end = _r3_independent_round_trip_light_time(fixture, provider, mid + half)
    return campaign.C_LIGHT_MPS * ((rho_end - rho_start) / float(interval_s)) / 2.0


def _r3_a07_rho_relative_budget(rho_s):
    """K * ulp(rho) / rho -- derived from the repaired arithmetic."""
    rho = abs(float(rho_s))
    return R3_A07_ORACLE_K_ULP_RHO * float(np.spacing(rho)) / rho


def _r3_a07_observable_budget_mps(rho_s, interval_s):
    """K * c * ulp(rho) / (2*Tc) -- the rho budget through the observable."""
    return (
        R3_A07_ORACLE_K_ULP_RHO
        * campaign.C_LIGHT_MPS
        * float(np.spacing(abs(float(rho_s))))
        / (2.0 * float(interval_s))
    )
...
def test_r3_p04a_endpoint_round_trip_light_times_match_model_s(real_fixture, interval_s):
    """R3-P04A (historical characterization after Owner Addendum 07).

    ORIGINAL PURPOSE: prove the event solve itself agrees with the accepted
    model-S implementation, free of the c/(2*Tc) amplification -- i.e. that the
    two implementations solve the same events.

    WHAT IT ACTUALLY MEASURED: agreement of ARITHMETIC ROUTE. Both sides formed
    the round-trip light time as ``t3 - t1``, so their shared ulp(epoch)
    conditioning error cancelled in the difference. The 1e-15 relative
    tolerance was calibrated against that cancellation.

    AFTER THE REPAIR: production assembles the round-trip light time from local
    intervals; model-S is byte-protected and still subtracts epochs. What this
    test now sees is model-S's own conditioning error, no longer cancelled.

    This node keeps its identity and keeps running -- the divergence is
    recorded, never hidden -- but its NUMERICAL-ACCURACY authority has moved to
    ``test_r3_p04a_a07_endpoint_light_times_match_the_independent_oracle``.
    """
    ...
    for label in ("start", "end"):
        ...
        # Characterization headroom, not an accuracy criterion (see the
        # Addendum-07 constant block).
        budget = R3_A07_MODEL_S_CHARACTERIZATION_K * float(
            np.spacing(abs(float(production[label])))
        ) / abs(float(production[label]))
        assert relative <= budget, (interval_s, label, relative, budget)


@pytest.mark.parametrize("interval_s", campaign.FROZEN_INTERVALS_S)
def test_r3_p04a_a07_endpoint_light_times_match_the_independent_oracle(
    real_fixture, interval_s
):
    """R3-P04A numerical accuracy, against INDEPENDENT truth (Addendum 07).

    This is the gate that now carries P04A's scientific purpose. Production's
    endpoint round-trip light times are compared against a structurally
    independent local-unknown root solve, inside the error budget derived from
    the repaired arithmetic (K = 5 ulp(rho)) -- not against model-S, and not
    against a tolerance chosen to pass.
    """
    fixture = real_fixture
    provider = _r3_reference_provider(fixture)
    production, _reference = _r3_endpoint_round_trip_light_times(fixture, interval_s)
    mid = float(fixture.spec.receive_mid_time_s)
    half = 0.5 * float(interval_s)
    for label, t3 in (("start", mid - half), ("end", mid + half)):
        truth = _r3_independent_round_trip_light_time(fixture, provider, t3)
        relative = abs(float(production[label]) - truth) / abs(truth)
        budget = _r3_a07_rho_relative_budget(truth)
        assert relative <= budget, (interval_s, label, relative, budget)


@pytest.mark.parametrize("interval_s", campaign.FROZEN_INTERVALS_S)
def test_r3_p04a_a07_production_is_no_worse_than_model_s_against_truth(
    real_fixture, interval_s
):
    """Owner Addendum 07 section 17 necessary condition, enforced in-suite.

    The supersession is only legitimate while repaired production is at least
    as close to independent truth as the historical model-S output. If that
    ever inverts, the authority transition is void and this fails.
    """
    fixture = real_fixture
    provider = _r3_reference_provider(fixture)
    production, reference = _r3_endpoint_round_trip_light_times(fixture, interval_s)
    mid = float(fixture.spec.receive_mid_time_s)
    half = 0.5 * float(interval_s)
    for label, t3 in (("start", mid - half), ("end", mid + half)):
        truth = _r3_independent_round_trip_light_time(fixture, provider, t3)
        err_production = abs(float(production[label]) - truth)
        err_model_s = abs(float(reference[label]) - truth)
        assert err_production <= err_model_s, (
            interval_s,
            label,
            err_production,
            err_model_s,
...
def test_r3_p04b_observable_difference_stays_inside_the_ulp_budget(real_fixture, interval_s):
    ...
    # Owner Addendum 07: characterization headroom against the byte-protected
    # model-S output, whose own ``t3 - t1`` conditioning error this difference
    # now exposes. Numerical accuracy is gated separately, against independent
    # truth, by test_r3_p04b_a07_observable_matches_the_independent_oracle.
    characterization_budget_mps = (
        R3_A07_MODEL_S_CHARACTERIZATION_K / R3_P04B_ULP_FACTOR
    ) * budget_mps
    assert absolute_difference <= characterization_budget_mps, (
        interval_s,
        absolute_difference,
        characterization_budget_mps,
...
@pytest.mark.parametrize("interval_s", campaign.FROZEN_INTERVALS_S)
def test_r3_p04b_a07_observable_matches_the_independent_oracle(real_fixture, interval_s):
    """R3-P04B numerical accuracy, against INDEPENDENT truth (Addendum 07).

    Carries P04B's scientific purpose: the amplified residue must stay inside
    the analytic budget. The budget is unchanged in FORM -- K*c*ulp(rho)/(2*Tc)
    -- and K is derived from the repaired arithmetic's rounding terms, not
    tuned. Only the reference moved, from model-S to independent truth.
    """
    fixture = real_fixture
    provider = _r3_reference_provider(fixture)
    produced = campaign.two_way_counted_doppler_observable(
    ...
    truth = _r3_independent_observable(fixture, provider, interval_s)
    mid = float(fixture.spec.receive_mid_time_s)
    rho = _r3_independent_round_trip_light_time(fixture, provider, mid)
    budget_mps = _r3_a07_observable_budget_mps(rho, interval_s)
    absolute_error = abs(float(produced) - truth)
    assert absolute_error <= budget_mps, (interval_s, absolute_error, budget_mps)


@pytest.mark.parametrize("interval_s", campaign.FROZEN_INTERVALS_S)
def test_r3_p04b_a07_production_is_no_worse_than_model_s_against_truth(
    real_fixture, interval_s
):
    """Owner Addendum 07 section 17 necessary condition, observable level."""
    fixture = real_fixture
    provider = _r3_reference_provider(fixture)
    produced = float(
        campaign.two_way_counted_doppler_observable(
            fixture.spec.receive_mid_time_s,
            fixture.station,
            fixture.t_grid_s,
            fixture.state_history_mci,
    ...
    truth = _r3_independent_observable(fixture, provider, interval_s)
    assert abs(produced - truth) <= abs(reference - truth), (
        interval_s,
        abs(produced - truth),
        abs(reference - truth),
    )
...
def test_r3_p04a_holds_across_the_frozen_transform_cadences(spice_epoch_s):
    ...
    Owner Addendum 07: the cadence sweep of R3-P04A. Same authority transition
    as the single-cadence gate -- model-S is now a historical characterization
    reference here, and numerical accuracy across cadences is gated against
    independent truth by the ``_a07_`` companion below.
    """
...
def test_r3_p04a_a07_cadence_sweep_matches_the_independent_oracle(spice_epoch_s):
    """Cadence sweep of the R3-P04A accuracy gate, against INDEPENDENT truth.

    The transform cadence changes how the station state is built, so the
    accuracy claim has to hold across cadences and not just at 3 s. Production
    is compared against the independent local-unknown solve inside the derived
    K = 5 ulp(rho) budget, and is additionally required to be no worse than
    model-S (Addendum 07 section 17).
    """
    for cadence_s in (0.1, 3.0, 60.0):
        fixture = campaign.build_campaign_fixture(
            campaign.frozen_geometry_specs()[0], cadence_s, et0_s=spice_epoch_s
        ...
        for interval_s in campaign.FROZEN_INTERVALS_S:
            ...
            for label, t3 in (("start", mid - half), ("end", mid + half)):
                ...
                err_production = abs(float(production[label]) - truth)
                err_model_s = abs(float(reference[label]) - truth)
                budget = _r3_a07_rho_relative_budget(truth) * abs(truth)
                assert err_production <= budget, (
                    cadence_s, interval_s, label, err_production, budget,
                )
                assert err_production <= err_model_s, (
                    cadence_s, interval_s, label, err_production, err_model_s,
                )
...
def test_r3_p11_production_exact_equals_s_and_zero_delay_f_minus_s_is_zero(real_fixture):
    ...
    # Zero transponder delay: F and S describe the same physics, so their
    # difference is a pure ARITHMETIC-ROUTE difference.
    #
    # Owner Addendum 07. Model-F composes the repaired M3 core (this module
    # imports lunar_od.two_way_range), while model-S keeps its own
    # ``float(t3 - t1)`` assembly at line 466 and is byte-protected. After the
    # Addendum-05 repair F is better conditioned than S, so F - S no longer
    # vanishes: it now measures S's epoch-subtraction error, and the previous
    # abs=1e-14 pin was only attainable while BOTH sides shared that error.
    #
    # The physical statement -- zero delay means F and S are the same model --
    # is unchanged and is now gated against independent truth by
    # test_r3_p11_a07_zero_delay_four_event_matches_the_independent_oracle.
    zero_delay_characterization_mps = (
        R3_A07_MODEL_S_CHARACTERIZATION_K / R3_P04B_ULP_FACTOR
    ) * budget_mps
    assert abs(decomposition.four_event_model_error) <= zero_delay_characterization_mps, (
        decomposition.four_event_model_error,
        zero_delay_characterization_mps,
    )
    assert (
        abs(decomposition.four_event_observable - decomposition.exact_station_observable)
        <= zero_delay_characterization_mps
    )


def test_r3_p11_a07_zero_delay_four_event_matches_the_independent_oracle(real_fixture):
    """R3-P11 numerical accuracy at zero delay, against INDEPENDENT truth.

    Carries P11's scientific purpose. At zero transponder delay the four-event
    model must reproduce the single-bounce physics; that is now asserted
    against a structurally independent local-unknown solve, inside the derived
    K = 5 ulp(rho) budget, rather than against model-S's binary64 output.

    Model-F is explicitly NOT fully numerically independent of production -- it
    composes the same repaired M3 core -- which is exactly why an external
    oracle is required here instead of an F-vs-S identity.
    """
    fixture = real_fixture
    interval_s = 3.0
    provider = _r3_reference_provider(fixture)
    ...
    error_f = abs(float(decomposition.four_event_observable) - truth)
    assert error_f <= budget_mps, (error_f, budget_mps)

    # Section 17 necessary condition at the F/S level.
    error_s = abs(float(decomposition.exact_station_observable) - truth)
    assert error_f <= error_s, (error_f, error_s)
