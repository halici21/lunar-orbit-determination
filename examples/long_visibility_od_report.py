"""Run SRIF OD on long SPICE-ITRF visibility arcs.

This is the first end-to-end visual chain:

    SPICE ephemeris -> truth propagation -> SPICE-ITRF visibility arcs
    -> synthetic measurements -> cold/hot SRIF arc sequence -> PNG/CSV report

Run from the project root:

    python python_port/examples/long_visibility_od_report.py
"""
...
def main() -> None:
    ...
    try:
        ...
        print(f"Building position-only measurements for {len(seg_starts)} SPICE-visibility arcs...")
    ...
    print(f"Running cold-start SRIF over {len(arcs)} arcs...")
    ...
    print("Running state hot-start SRIF...")
    ...
    csv_path = write_scenario_summary_csv([cold, hot], out_dir / "long_visibility_position_od_summary.csv")
    fig_path = plot_scenario_comparison(
        [cold, hot],
        out_dir / "long_visibility_position_od_comparison.png",
        title="Long SPICE-Visibility Position-Only SRIF Cold vs State Hot-Start",
    )
